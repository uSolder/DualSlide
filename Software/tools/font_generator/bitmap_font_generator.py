...
The generated glyph bitmaps:
    ...
    - cover one contiguous Unicode range by default, or a selected character
      list when --characters is supplied.
"""
...
def render_font_definition(
    ...
    codepoints_name = f"{public_name}{font.size}_GlyphCodepoints"
    ...
    if is_sparse:
        codepoint_table = (
            f"static const uint32_t {codepoints_name}[] =\n"
            "{\n"
            + ",\n".join(f"    {codepoint}U" for codepoint in codepoints)
            + "\n};\n"
        )
        glyph_codepoints = codepoints_name
        first_codepoint = 0
    else:
        codepoint_table = ""
        glyph_codepoints = "NULL"
...
{{
    .bitmap = {bitmap_name},
    .glyphs = {glyphs_name},
    .glyphCodepoints = {glyph_codepoints},
...
def render_source(
    ...
    definitions = "\n\n".join(
        render_font_definition(font, public_name, codepoints, is_sparse)
        for font in fonts
    )
 ...
 *
 * Source characters: {"selected list" if is_sparse else f"U+{codepoints[0]:04X} through U+{codepoints[-1]:04X}"}
...
def generate_files(
    ...
    if characters is None:
        codepoints = tuple(range(first_codepoint, last_codepoint + 1))
        is_sparse = False
    else:
        if not characters:
            raise ValueError("--characters must contain at least one character.")

        # Retain first appearance order and omit repeats; one glyph per character.
        codepoints = tuple(dict.fromkeys(ord(character) for character in characters))
        is_sparse = True
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--characters",
        help=(
            "Generate only these characters, rather than the inclusive "
            "codepoint range. Repeated characters are emitted once."
        ),
    )
    parser.add_argument(
        "--output-directory",
